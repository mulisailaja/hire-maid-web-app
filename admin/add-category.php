...
if (isset($_POST['submit'])) {
    ...
    $perhouramount = $_POST['perhouramount'];
    $monthlyamount = $_POST['monthlyamount'];

    $sql = "INSERT INTO tblcategory(CategoryName, PerHourAmount, MonthlyAmount) VALUES(:catname, :perhouramount, :monthlyamount)";
    $query = $dbh->prepare($sql);
    $query->bindParam(':catname', $catname, PDO::PARAM_STR);
    $query->bindParam(':perhouramount', $perhouramount, PDO::PARAM_STR);
    $query->bindParam(':monthlyamount', $monthlyamount, PDO::PARAM_STR);

    $query->execute();

    $LastInsertId = $dbh->lastInsertId();
    if ($LastInsertId > 0) {
        echo '<script>alert("Category and amounts have been added.")</script>';
...
<html lang="en">
   ...
   <body class="inner_page general_elements">
    ...
    <input type="text" name="perhouramount" value="" class="form-control" required='true'>
</div>
<div class="field">
    <label class="label_field">Monthly Amount</label>
    <input type="text" name="monthlyamount" value="" class="form-control" required='true'>
</div>
